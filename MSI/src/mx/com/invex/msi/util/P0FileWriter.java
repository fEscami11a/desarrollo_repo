...
public class P0FileWriter {
	private static final int BUFFER_SIZE = 1024;
    private static final int DIGEST_BASE_RADIX = 16;
    private static final String LINE_SEPARATOR = System.getProperty("line.separator");
    private static HashMap<String,P0FileWriter> sLoggerMap;
     ...
     */
    private P0FileWriter(String logDir, String logFileName) throws IOException {
        mLogDir = logDir;
     ...
     *             Thrown if the file could not be created or opened for writing.
     */
    public static P0FileWriter getLog(String logDir, String logFileName) throws IOException {
        if(logDir == null || logFileName == null || logDir.isEmpty() || logFileName.isEmpty()) {
            return null;
        }

        if(sLoggerMap == null) {
            sLoggerMap = new HashMap<String, P0FileWriter>();
        }

        final String logFilePathHash = getHash(logDir + File.separatorChar + logFileName);
        if(!sLoggerMap.containsKey(logFilePathHash)) {
            sLoggerMap.put(logFilePathHash, new P0FileWriter(logDir, logFileName));
        }
